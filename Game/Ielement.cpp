...
bool IElement::check(int onex, int oney, int twox, int twoy) {
	if (((twox - onex)*(twox - onex) + (twoy - oney)*(twoy - oney))==1)
		return true;
	else
		return false;
}
void IElement::del(int delx, int dely,Board* p) {
	if ((*p).field[delx][dely + 1]==symbol)
		
	{
		if ((*p).field[delx][dely + 2] == symbol)
		{
	...
	if ((*p).field[delx - 1][dely] == symbol)
	{
		if ((*p).field[delx - 2][dely] == symbol)
		{
			...
			else
			{
				...
				(*p).checkfield[delx - 2][dely] = '1';
			}
		}
		else if ((*p).field[delx + 1][dely] == symbol)
		{
